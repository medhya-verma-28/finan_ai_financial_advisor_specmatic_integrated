...
result = lx.extract(
            text_or_documents="QUERY_PLACEHOLDER",
            prompt_description=lx_prompt,
            examples=lx_examples,
            model_id="gemini-2.5-flash",
        )

extracted_features = {col:result.extractions[i].text for i,col in enumerate(lx_classes) if col in [extraction.class_ for extraction in result]}

numeric_keys = ['Monthly Income (INR)', 'Cost of Living Expenditure (INR)', 'Other Important Investments (INR)',
                'Consumerist Expenditure (INR)', 'Crisis Shock Expenditure (INR)', 'Total Monthly Expenditure (INR)']
numeric_vals = [int(extracted_features[key]) for key in numeric_keys]
# --- Model 1: Financial State Category ---
X_val_df = pd.DataFrame(
    {col: [numeric_vals[i]] for i, col in enumerate(numeric_keys)}
)
X_val_df['Debt Status'] = extracted_features['Debt Status']
...
X2_val_df['Debt Status'] = [extracted_features['Debt Status']]
...
suggested_cost_of_living_percentage = model3.predict(X3_transformed_val)
suggested_other_investments_percentage = model4.predict(X3_transformed_val)
suggested_consumerist_expenditure_percentage = model5.predict(X3_transformed_val)
suggested_crisis_shocks_percentage = model6.predict(X3_transformed_val)

suggested_cost_of_living = suggested_cost_of_living_percentage[0] * extracted_features['Monthly Income (INR)']
suggested_other_investments = suggested_other_investments_percentage[0] * extracted_features['Monthly Income (INR)']
suggested_consumerist_expenditure = suggested_consumerist_expenditure_percentage[0] * extracted_features['Monthly Income (INR)']
suggested_crisis_shocks = suggested_crisis_shocks_percentage[0] * extracted_features['Monthly Income (INR)']
...
Monthly Income (INR): {extracted_features['Monthly Income (INR)']}
Cost of Living Expenditure (INR): {extracted_features['Cost of Living Expenditure (INR)']}
Other Important Investments (INR): {extracted_features['Other Important Investments (INR)']}
Consumerist Expenditure (INR): {extracted_features['Consumerist Expenditure (INR)']}
Crisis Shock Expenditure (INR): {extracted_features['Crisis Shock Expenditure (INR)']}
Total Monthly Expenditure (INR): {extracted_features['Total Monthly Expenditure (INR)']}
Debt Status: {extracted_features['Debt Status']}
...
Suggested Budget for Cost of Living Expenditure (INR): {(suggested_cost_of_living):.2f}
Suggested Budget for Other Important Investments (INR): {(suggested_other_investments):.2f}
Suggested Budget for Consumerist Expenditure (INR): {(suggested_consumerist_expenditure):.2f}
Suggested Budget for Crisis Shock Expenditure (INR): {(suggested_crisis_shocks):.2f}
